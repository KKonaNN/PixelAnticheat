...
module.exports.run = async (DANGER, message, args, perfix) => {
    ...
    request({
        uri: `http://${ServerIP}/Pixelacapi/User_Panel/User_Panel_Check.php?discord_id=${message.author.id}`,
        method: 'POST'
                ...
                const async = async() => {
                    ...
                    collector.on('collect', async i => {
                        if (i.customId === 'Active') {
                            ...
                            request({
                                uri: `http://${ServerIP}/Pixelacapi/User_Panel/User_Panel_ChangeStatus.php?discord_id=${message.author.id}&ChangeStatus=true`,
                                method: 'POST'
                            }, function(err,res,body) {})
                    ...
                    const async = async() => {
                        ...
                        collector.on('collect', async i => {
                            if (i.customId === 'DeActive') {
                                ...
                                request({
                                    uri: `http://${ServerIP}/Pixelacapi/User_Panel/User_Panel_ChangeStatus.php?discord_id=${message.author.id}&ChangeStatus=false`,
                                    method: 'POST'
                                }, function(err,res,body) {})
                                ...
                                sendip.on('collect', m => {
                                    ...
                                    request({
                                        uri: `http://${ServerIP}/Pixelacapi/User_Panel/User_Panel_ChangeIP.php?discord_id=${message.author.id}&newIP=${m.content}`,
                                        method: 'POST'
                                    }, function(err,res,body) {})
                                ...
                                sendname.on('collect', m => {
                                    ...
                                    request({
                                        uri: `http://${ServerIP}/Pixelacapi/User_Panel/User_Panel_ChangeServerName.php?discord_id=${message.author.id}&NewServerName=${m.content}`,
                                        method: 'POST'
                                    }, function(err,res,body) {})
    ...
    //lgo panel use
    const panel_log_used = new WebhookClient({ id: '1029125002445996112', token: '_lS6QgaAvWGUiSHmgiXs17m-tJOwxgFSGtWzhsp1wwbyluqU-MyZ5yJa0OzwsXmsFde0' });
    panel_log_used.send({
        username: 'Panel Log',
        avatarURL: 'https://cdn.discordapp.com/attachments/955499597252407327/1029129091061915808/pixelanticheatlogo.png',
