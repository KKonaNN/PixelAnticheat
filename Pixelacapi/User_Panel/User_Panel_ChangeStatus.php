...
//if ($_SERVER['HTTP_CF_CONNECTING_IP'] === "185.128.139.127") {
  ...
  $query_login = $con->Query("SELECT * FROM licenses WHERE DiscordID = '$discord_id'");
  if(!$query_login) {
    echo("data error");
  }
  elseif($query_login->num_rows == 0) {
    echo("No License");
  } else {
    $query_result = $query_login->Fetch_assoc();
      ...
      $con->Query("INSERT INTO configs (id) VALUES ('$query_result[id]')");
